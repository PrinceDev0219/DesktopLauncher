...
void CMFCLinkCtrl::OnDraw(CDC* pDC, const CRect& rect, UINT /*uiState*/)
{
	...
	if (m_bAlwaysUnderlineText)
	{
		//pOldFont = pDC->SelectObject(&(GetGlobalData()->fontDefaultGUIUnderline));
		CFont	fn;
		
		fn.CreateFontW(rect.Height(), rect.Width()/6, 0, 0, 0, 0, 1, 0, ANSI_CHARSET, 0, 0, 0, 0, L"sdfasdf");
		pOldFont = pDC->SelectObject(&fn);
	}
	else
	{
	...
	// Set text parameters:
	pDC->SetTextColor(m_bHover ? GetGlobalData()->clrHotLinkHoveredText : (m_bVisited ? GetGlobalData()->clrHotLinkNormalText : GetGlobalData()->clrHotLinkNormalText));
